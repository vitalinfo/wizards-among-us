...
- **Child-data exposure (three tiers — never widen a tier):**
  1. **Browse card** (any signed-in volunteer, pre-claim): child *first* name, age, **`home_region` + `home_town`** (where they came from), **`current_region`** (oblast only), gift description + price, and **`family_story`**. Nothing else. Enforced by `toBrowseCard`.
     **`current_town` is NOT on the card** and must not be added. Origin is where a family no longer is, so it locates nobody; the current town is where the child can actually be found, and together with a first name, an age and a family story it is enough to identify one displaced family in a small place. It stays tier 2.
     The story is on the card by a **deliberate decision** (Vital, Phase 6): a volunteer chooses *which* child to help, and the family's own account is what informs that choice, so withholding it until after the commitment inverts the order. It matches what the parent form already promises — «це читає чарівник, який обиратиме дитину». It is the one free-text, parent-written field any signed-in user can read (there is no volunteer approval gate), so if abuse appears this is the first thing to reconsider.
