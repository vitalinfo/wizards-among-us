...
- **Child-data exposure (three tiers — never widen a tier):**
  1. **Browse card** (any signed-in volunteer, pre-claim): child *first* name, age, current region, gift description + price, and **`family_story`**. Nothing else. Enforced by `toBrowseCard`.
