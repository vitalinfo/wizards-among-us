...
export const campaigns = pgTable(
  ...
  (t) => [
    ...
    // At most one 'active' campaign at a time (invariant).
...
// Single-row table holding the global kill switch (orthogonal to campaign
// status). `id` is a boolean pinned to TRUE so only one row can ever exist.
export const settings = pgTable(
  "settings",
  {
    id: boolean("id").primaryKey().default(true),
    applicationsEnabled: boolean("applications_enabled")
      .notNull()
      .default(true),
  ...
  (t) => [check("settings_singleton", sql`${t.id}`)],
);
...
// enforced at SUBMIT via zod, not NOT NULL.
export const applications = pgTable(
  ...
  (t) => [
    ...
    check(
      "applications_child_age_valid",
      sql`${t.childAge} is null or ${t.childAge} between 0 and 18`,
    ),
...
export const claims = pgTable(
  ...
  (t) => [uniqueIndex("claims_application_unique").on(t.applicationId)],
);
...
// so rows stay readable after an actor is deleted.
// ---------------------------------------------------------------------------
