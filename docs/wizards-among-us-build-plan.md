...
6. ~~**Exact base field set**~~ — **resolved from the real 2025 form**: the existing base fields plus `contact`/`contact_method` (Telegram handle *or* UA phone), `gift_url`, `consent_at`, and `social_media_consent`; `campaigns.gift_price_cap` carries the per-campaign budget ceiling (2025: 700 UAH). Age band is **derived** from `child_age`, not asked twice. Three uploads: `idp_certificate`, `letter_photo`, `child_with_letter_photo`. Remaining: the `new_school_year` type fields (first campaign is St Nicholas).
